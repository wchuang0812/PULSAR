...
int main(int argc, char *argv[])
{
    ...
    int candnum, numrfi = 0, numrfivect = NUM_RFI_VECT;
    ...
    if (!cmd->nocomputeP) {
        ...
        double *power_result = (double *)malloc((harmsum+1)*sizeof(double));
        for (ii = 1; ii <= harmsum; ii++) {
            power_result[ii] = power_for_sigma(cmd->freqsigma, ii, ptsperint / 2 - lobin);
        }
        int proc = cmd->ncpus;
        int numcands[numchan];
        fftcand *cands[numchan];
        float *chandata[numchan];
        int firsttime = 1;
        for (ii = 0; ii < numchan; ii++) {
             cands[ii] = NULL;
             chandata[ii] = gen_fvect(ptsperint);
        }
        //double davg, dvar;
        //float norm = 0.0;
        //float powavg, powstd, powmax;
        ...
        for (ii = 0; ii < numint; ii++) {       /* Loop over the intervals */
            ...
            int start_loop = 0, end_loop = numchan;
            if (firsttime) {
                 end_loop = start_loop + 1;
                 firsttime = 0;
            }
            while(start_loop < numchan){
               if(end_loop > numchan) end_loop = numchan;         
               #ifdef _OPENMP
               #pragma omp parallel for shared(start_loop, ii, dataavg, datastd, datapow, rawdata, s, blocksperint, ptsperint, power_result, lobin, harmsum, numbetween, interptype)
               #endif
               for (jj = start_loop; jj < end_loop; jj++) {  /* Loop over the channels */
                    double davg, dvar;
                    float powavg, powstd, powmax;
                    float norm = 0.0;
                    powmax = 0.0;
                    get_channel(chandata[jj], jj, blocksperint, rawdata, &s);
                    /* Calculate the averages and standard deviations */
                    /* for each point in time.                        */
                    avg_var(chandata[jj], ptsperint, &davg, &dvar);     
                    dataavg[ii][jj] = davg;
                    datastd[ii][jj] = sqrt(dvar);
                    numcands[jj] = 0;       
                    // Don't search the power spectrum if there is little to no variance
                    if (datastd[ii][jj] > 1e-4) {
                        realfft(chandata[jj], ptsperint, -1,jj + ii * numchan);
                        norm = datastd[ii][jj] * datastd[ii][jj] * ptsperint;
                        cands[jj] = search_fft((fcomplex *) chandata[jj], ptsperint / 2,
                                           lobin, ptsperint / 2, harmsum,
                                           numbetween, interptype, norm, cmd->freqsigma,
                                           &numcands[jj], &powavg, &powstd, &powmax, power_result);
               ...
               for (jj = start_loop; jj < end_loop; jj++)
               {         
                    /* Record the birdies */ 
                    if (numcands[jj]) {
                        for (kk = 0; kk < numcands[jj]; kk++) {
                            freq = cands[jj][kk].r / inttime;
                            candnum =
                                find_rfi(rfivect, numrfi, freq, RFI_FRACTERROR);
                            if (candnum >= 0) {
                                update_rfi(rfivect + candnum, freq, cands[jj][kk].sig,
                                           jj, ii);
                            } else {
                                update_rfi(rfivect + numrfi, freq, cands[jj][kk].sig, jj,
                                           ii);
                                numrfi++;
