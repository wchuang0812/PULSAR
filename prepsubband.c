...
int main(int argc, char *argv[])
{
    ...
    if (cmd->nobaryP) {         /* Main loop if we are not barycentering... */
        ...
        if (cmd->subP)
            ...
            outdata = gen_fmatrix(cmd->numdms, worklen / cmd->downsamp);
        //printf("s->num_subint is %d %d %d %d\n",s.num_subint[0],s.num_subint[1],s.num_subint[2],s.num_subint[3]);
        numread = get_data(outdata, blocksperread, &s,
                           &obsmask, idispdt, offsets, &padding, subsdata);
        while (numread == worklen) {    /* Loop to read and write the data */
           //printf("padding is %d numread is %d worklen is %d\n",padding,numread,worklen);
            int numwritten = 0;
...
{   
    ...
    static float *current[28];
    static fftwf_plan tplan1, tplan2;    
    int  numsubbands = cmd->nsub;
    ...
    if (firsttime) {
        ...
        for (ii = 0; ii < 28; ii++)
            current[ii] = NULL;
        for (ii = 0; ii < cmd->ncpus; ii++) 
            current[ii] = gen_fvect(s->num_channels * s->spectra_per_subint);
        
        last = gen_fvect(s->num_channels * s->spectra_per_subint);
        ...
        tplan1 = plan_transpose(s->spectra_per_subint, s->num_channels, last, last);
        tplan2 = plan_transpose(numsubbands, s->spectra_per_subint, currentdata, currentdata);
        //printf("blocksperread is %d\n",blocksperread);
    }
    int start0 = 0;
    while (1) {
        if (RAWDATA || insubs) {
            if (firsttime){
                s->get_rawblock(last, s, padding);
                int nummasked0 = 0;
                int offset;
                nummasked0 = check_mask(0, s->time_per_subint, obsmask, maskchans);
                int channum;
                for (kk = 0; kk < s->spectra_per_subint; kk++) {
                     offset = kk * s->num_channels;
                     for (jj = 0; jj < nummasked0; jj++) {
                          channum = maskchans[jj];
                          last[offset + channum] = s->padvals[channum];
                     }
                }
                fftwf_execute_r2r(tplan1, last, last);           
            }
            int num_threads;
            int tt = start0, end0;
//struct  timeval end, start;
//double  timeperform = 0;
//gettimeofday(&start,NULL);
            while (tt < blocksperread) {
               if (cmd->ncpus > 1) {
                   end0 = tt + cmd->ncpus;
                   if (end0 > blocksperread) 
                        end0 = blocksperread;
                   if ((cs + tt) % 12 == 0 || (cs + tt) % 12 == 11) 
                        end0 = tt + 1;
                   else {
                        if (end0 > tt + 11 - (cs + tt) % 12) 
                             end0 = tt + 11 - (cs + tt) % 12;
                   }
               }
               else 
                   end0 = tt + 1;
               num_threads = end0 - tt;
               for (ii = tt; ii < end0; ii++)  
                   s->get_rawblock(current[ii-tt], s, padding);
               #ifdef _OPENMP
               #pragma omp parallel for num_threads(num_threads) shared(current,cs,s,maskchans,obsmask,padding)
               #endif           
               for (ii = tt; ii < end0; ii++) { 
                   double starttime = 0.0;
                   int nummasked0 = 0;
                   int offset;
                   starttime = (cs+ii) * s->spectra_per_subint * s->dt;
                   nummasked0 = check_mask(starttime, s->time_per_subint, obsmask, maskchans);
                   int channum;
                   for (kk = 0; kk < s->spectra_per_subint; kk++) {
                        offset = kk * s->num_channels;
                        for (jj = 0; jj < nummasked0; jj++) {
                             channum = maskchans[jj];
                             current[ii-tt][offset + channum] = s->padvals[channum];
                        }
                   }
                   fftwf_execute_r2r(tplan1, current[ii-tt], current[ii-tt]);
               }
               #ifdef _OPENMP
               #pragma omp parallel for num_threads(num_threads) shared(current,blocksize,idispdts,cmd,s,obsmask,padding)
               #endif
               for (ii = tt; ii < end0; ii++) {
                   if (ii == tt) {
                        dedisp_subbands(current[ii-tt], last, s->spectra_per_subint,
                                                 s->num_channels, idispdts, numsubbands, currentdata + ii * blocksize);
                   } else {
                        dedisp_subbands(current[ii-tt],current[ii-tt-1], s->spectra_per_subint,
                        s->num_channels, idispdts, numsubbands, currentdata + ii * blocksize);
                   }
                   fftwf_execute_r2r(tplan2, currentdata + ii * blocksize, currentdata + ii * blocksize);
                   if (tmppad)
                      *padding = 1;
                   if (!firsttime)
                      totnumread += s->spectra_per_subint;
               }     
               last = current[end0 - tt - 1];   
               tt = end0;
            }  
//gettimeofday(&end,NULL);
//timeperform = (end.tv_sec-start.tv_sec)+(end.tv_usec-start.tv_usec)/1000000.0;
//printf("The total time of while is %f !\n",timeperform);
            cs += blocksperread;  
        }
...
//gettimeofday(&start,NULL);
    if (!cmd->subP) {
        #ifdef _OPENMP
        #pragma omp parallel for num_threads(cmd->ncpus) shared(currentdsdata,lastdsdata,dsworklen,offsets,outdata)
        #endif
        for (ii = 0; ii < cmd->numdms; ii++)
...
//gettimeofday(&end,NULL);
//timefloat = (end.tv_sec-start.tv_sec)+(end.tv_usec-start.tv_usec)/1000000.0;
//printf("The total time of float dedisp is %f !\n",timefloat);
