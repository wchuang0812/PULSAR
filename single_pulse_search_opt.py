...
def parallel_func(index, filelist, opts, useffts, dosearch, pgplot_device, fftlen, chunklen, detrendlen, blocks_per_chunk, overlap, worklen, max_downfact, default_downfacts, filenmbase):
    candfile = open("cand" + str(index) + '.txt', mode='w')
    DMfile = open("DM" + str(index) + '.txt', mode='w')  
    candlist = []
    num_v_DMstr = {}
    if useffts:
    ...
    for filenm in filelist:
            ...
            if len(dm_candlist):
                ...
                for cand in dm_candlist:
                    ...
                    candfile.write(str(cand.DM)+' '+str(cand.sigma)+' '+str(time)+' '+str(bin)+' '+str(downfact)+'\n')
            outfile.close()

            # Add these candidates to the overall candidate list
            #num_v_DMstr[DMstr] = len(dm_candlist)
            DMfile.write(DMstr + " " + str(len(dm_candlist)) + "\n")
    candfile.close()
    DMfile.close()
...
def main():
    ...
    if not dosearch:
        ...
        ii = 0
        for filenm in args:
            Filelist[ii%proc].append(filenm)
            ii += 1
       # q1 = multiprocessing.Queue()
       # q2 = multiprocessing.Queue()
        for i in range(proc):
            p = multiprocessing.Process(target=parallel_func, args=[i, Filelist[i], opts, useffts, dosearch, pgplot_device, fftlen, chunklen, detrendlen, blocks_per_chunk, overlap, worklen, max_downfact, default_downfacts, filenmbase])
            jobs.append(p)
        for p in jobs:
            p.start()
        for p in jobs:
            p.join()
      #  results = [q1.get() for j in jobs]
      #  for res in results:
      #      for candd in res:
      #          candlist.append(candd)
      #  results = [q2.get() for j in jobs]
      #  for res in results:
      #      for key in res.keys():
      #          num_v_DMstr[key] = res[key]
               
    if (opts.makeplot):
        for index in range(proc):
            DMfile = open("DM"+str(index)+".txt")
            candfile = open("cand"+str(index)+".txt")
            for line in candfile.readlines():
                 linelist = line[:-1].split(' ')
                 candlist.append(candidate(float(linelist[0]),float(linelist[1]),float(linelist[2]),int(linelist[3]),int(linelist[4]))) 
            for line in DMfile.readlines():
                 linelist = line[:-1].split(' ')
                 num_v_DMstr[linelist[0]] = int(linelist[1])
