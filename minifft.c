...
fftcand *search_fft(fcomplex * fft, int numfft, int lobin, int hibin,
                    ...
                    float *powavg, float *powvar, float *powmax, double *power_result)
    ...
    if (numharmsum > 1) {
        ...
        for (ii = 2; ii <= numharmsum; ii++) {
            ...
            if (dynamic) {
                ...
                minpow = power_result[ii];
            }
            else 
                minpow = power_for_sigma(tmpminsig, ii, hibin - lobin);
            //printf("minpow is %f sigma is %f hibin-lobin is %d  i is %d \n",minpow,sigmacutoff,hibin - lobin,ii);  
